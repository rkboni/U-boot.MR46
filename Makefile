...
cmd_mkheader = $(PYTHON) scripts/elftombn.py -f $< -o $@

u-boot.mbn: u-boot.elf
...
# Version string put at the beginning of u-boot binary
ifeq ($(CONFIG_MERAKI_VERSION),y)
VER_COMMIT=$(shell git rev-parse --short HEAD)
ifeq ($(VER_COMMIT), )
$(error Failed to get git head hash)
endif
VER_TIME=$(shell [[ $$(date +%Y%m%d%H%M%S) =~ ^[0-9]{14}$$ ]] && echo $$(date +%Y%m%d%H%M%S))
ifeq ($(VER_TIME), )
$(error Failed to get timestamp)
endif
IKAREM_U_BOOT_VERSION="$(CONFIG_MVER_BOARD)-$(CONFIG_MVER_KEYWORD)-$(CONFIG_MVER_MAJOR).$(CONFIG_MVER_MINOR)-$(VER_COMMIT)-$(VER_TIME)"
endif

define filechk_version.h
	(echo \#define IKAREM_U_BOOT_VERSION \"$(IKAREM_U_BOOT_VERSION)\"; \
