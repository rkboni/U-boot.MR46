...
#define EEPROM_I2C_SCL_WRITE_STATUS 0x203
...
static int read_board_data(void)
{
 ...
 retry:
    for (i = 0; i < NUM_VALID_EEPROM_CONFIG; i++) {
        ...
        if (recover_i2c)
            toggle_scl_universal(valid_eeprom_i2c_config[i].gpio_0, valid_eeprom_i2c_config[i].gpio_1, EEPROM_I2C_SCL_WRITE_STATUS);
...
{
    if (this_cryptid) {
        env_set("itb_config", this_cryptid->itb_config);
        env_set("bootcmd", this_cryptid->bootcmd);
    }
}
