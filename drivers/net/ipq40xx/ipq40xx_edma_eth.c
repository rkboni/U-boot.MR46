...
*/
	status = phy_get_ops->phy_get_link_status(priv->mac_unit, 0);
	if (status == 1){
		// no link
		linkup = -1;
		printf("No link !\n");
	...
	phy_get_ops->phy_get_speed(priv->mac_unit, 0, &speed);
	phy_get_ops->phy_get_duplex(priv->mac_unit, 0, &duplex);
