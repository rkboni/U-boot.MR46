			     ...
			     struct mtd_oob_ops *ops)
{
	...
	/* We don't support MTD_OPS_PLACE_OOB as of yet. */
	...
	if (!ecc_error && mtd->ecc_stats.corrected != corrected)
		ret = -EUCLEAN;

done:
	ipq_exit_raw_mode(mtd);
	return (!ret && ecc_error) ? -EBADMSG : ret;
}
	...
	if ((from & (mtd->writesize - 1)) == 0) {
	   ...
	   *retlen = ops.retlen;
		...
		for (;;) {
			 ...
			 */
			ret = ipq_nand_read_oob(mtd, aligned_from, &ops);
...
static const struct nand_flash_dev *flash_get_dev(uint8_t dev_id)
{
	...
	for (i = 0; nand_flash_ids[i].id; i++) {
		if (nand_flash_ids[i].id[1] == dev_id)
...
int ipq_nand_init(enum ipq_nand_layout layout)
{
	uint32_t status;
	int ret;
	struct nand_chip *chip = &nand_chip[CONFIG_IPQ_NAND_NAND_INFO_IDX];
	...
	ret = nand_register(CONFIG_IPQ_NAND_NAND_INFO_IDX, mtd);
	if (ret < 0) {
