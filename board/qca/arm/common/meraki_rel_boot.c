...
/* fuse data included by each platform config */
extern const unsigned char sec_dat[];
extern const unsigned int sec_dat_len;
...
static bool devel_crt_valid(void)
{
   ...
   if (run_command("run check_dev_unlock", 0) != CMD_RET_SUCCESS) {
      printf("Invalid developer certificate\n");
      return false;
   }

   printf("Valid signed developer certificate\n");
   dev_crt = (const void *)env_get_ulong("imgaddr", 16, 0);
   if (!dev_crt) {
...
static void force_secboot(void)
{
   ...
   printf("\n\n");
   ret = qca_scm_call(SCM_SVC_FUSE, QFPROM_IS_AUTHENTICATE_CMD, &buf, sizeof(buf));
   if (ret != CMD_RET_SUCCESS || buf != 1) {
      ...
      void *fuseaddr = (void *)env_get_ulong("fuseaddr", 16, 0);
      if (!fuseaddr) {
         printf("Bad fuseaddr %p\n", fuseaddr);
         hang();
      }

      memcpy(fuseaddr, sec_dat, sec_dat_len);

      /* Blow fuses */
      printf("Blowing fuses...\n");
      ret = run_command("fuseipq $fuseaddr", 0);
      if (ret != CMD_RET_SUCCESS)
      ...
      /* just in case post_fuse_action() comes back */
      printf("Post-fuse action failed - halting!\n");
      hang();
   }
