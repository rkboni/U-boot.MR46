...
struct dumpinfo_t dumpinfo_s[] = {
	...
	{ "EBICS_S1.BIN", IPQ_TZ_END_ADDR, 0x10000000, 0 },
	{ "DATARAM.BIN", 0x00290000, 0x00010000, 0 },
	{ "MSGRAM.BIN", 0x00060000, 0x00006000, 1 },
	{ "IMEM.BIN", 0x08600000, 0x00006000, 0 },
	{ "NSSIMEM.BIN", 0x08600658, 0x00060000, 0, 1, 0x2000 },
...
{
	int tlmm_base = 0x102C000; // phy reset gpio:44
	...
	writel(7, tlmm_base + 0x18000);
	writel(7, tlmm_base + 0x19000);
...
void fdt_fixup_auto_restart(void *blob)
{
	...
	paniconwcssfatal = env_get("paniconwcssfatal");
...
{
	int orange_gpio = -1, node;
	unsigned int *orange_gpio_base;
	int delay = 100; /* ms */
	int i;

	node = fdt_path_offset(gd->fdt_blob, "/leds");
	if (node >= 0)
		orange_gpio = fdtdec_get_uint(gd->fdt_blob, node, "orange_gpio", -1);

	if (orange_gpio >=0) {
		orange_gpio_base = (unsigned int *)GPIO_CONFIG_ADDR(orange_gpio);

		seconds *= 1000 / (delay * 2);
		for (i = 0; !seconds || i < seconds; i++) {
			gpio_set_value(orange_gpio, 0x0);
			mdelay(delay);
			gpio_set_value(orange_gpio, 0x1);
			mdelay(delay);
		}
	}
}

/* after blowing fuses, axes can reboot into secure boot */
