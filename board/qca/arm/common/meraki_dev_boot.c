...
static int do_flash_power_led(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
   flash_power_led(5);
   return 0;
}
...
U_BOOT_CMD(
	flash_led,	1,	0,	do_flash_power_led,
...
static int do_meraki_bootcustom(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
   ...
   size = env_get_ulong("filesize", 16, -1);
